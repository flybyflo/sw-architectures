...
Consistency defines the system guarantees regarding state uniformity across multiple replicas or nodes at any given point in time. It spans from strong consistency (where all reads return the most recent write immediately, e.g., write-through caching, transactional databases) to eventual consistency (where replicas converge gradually after a delay, e.g., message brokers, git clones).
