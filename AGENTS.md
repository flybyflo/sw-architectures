...
Use the Architecture Wiki as a knowledge base in this order:

1. **Read `wiki/index.md` first** as the primary navigation source to locate
   relevant reviewed architecture documentation.
2. Open the most relevant reviewed pages under `wiki/` before using raw notes.
3. **Use QMD retrieval** when the index does not clearly identify the right
   pages, when the question needs semantic matching, or when comparing multiple
   projects, components, patterns, or quality attributes.
4. Prefer reviewed wiki pages over raw sources. Use `raw_sources/` notes only
   for verification fallback or missing reviewed coverage.
5. Cite exact Markdown file paths for every architectural claim in the output.
6. Do not invent missing information. If the wiki and raw notes do not contain
   enough support, say that the wiki does not contain enough information to
   answer the question.

QMD commands for agents:

```bash
qmd status
qmd query "<architecture question>" -c architecture-wiki -n 8 --no-rerank --full-path
qmd search "<exact term or component>" -c architecture-wiki --full-path
qmd query "<fallback verification question>" -c raw-sources -n 8 --no-rerank --full-path
qmd get qmd://architecture-wiki/path/to/page.md
```

For broader comparisons, use typed QMD queries so lexical terms and semantic
intent are both explicit:

```bash
qmd query $'lex: api gateway caching sharding\nvec: compare scalability and reliability tradeoffs across patterns' -c architecture-wiki --no-rerank --full-path
```

After adding or changing wiki or raw-source Markdown, refresh retrieval before
validating answers:
...
For detailed response layout specifications and constraint edge-cases, see
`docs/answering-rules.md`. The expected answer shape is:
...
Pages consulted:
- [Markdown paths checked through index/search/QMD retrieval]

Answer:
[Architecture synthesis]

Sources used:
- [Exact Markdown paths that support the claims]
```
